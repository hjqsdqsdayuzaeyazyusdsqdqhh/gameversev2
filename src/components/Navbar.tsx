              ...
              <AnimatePresence>
                {showCategories && (
                  <motion.div
                    ...
                    className="absolute top-full left-0 mt-2 w-[600px] p-3 rounded-2xl glass border border-dark-600/50 shadow-2xl shadow-black/40 grid grid-cols-4 gap-1"
                  >
                    {categories.map((cat) => (
                      <Link
                        key={cat.name}
      ...
      <AnimatePresence>
        {mobileMenu && (
          <motion.div
            ...
            className={`md:hidden border-t overflow-hidden ${theme === "dark" ? "border-dark-700/50 bg-dark-900/95 backdrop-blur-lg" : "border-gray-200 bg-white/95 backdrop-blur-lg"}`}
          >
